...
- **Scope**: Guide AI changes for the Zephyr-based smart irrigation firmware targeting nRF52840 and native simulation builds.
...
## BLE Layer
- **Primary Service**: `bt_irrigation_service.c` implements 33 characteristics with hard-coded attribute indices (`ATTR_IDX_*`); inserting/removing characteristics requires renumbering the constants and keeping docs aligned.
...
## Build & Test Workflow
- **Hardware Build**: `west build -b nrf52840_promicro --pristine` (use `boards/promicro_52840.overlay` automatically via CMake); flash with `west flash`.
- **Native Simulation**: `west build -b native_sim --overlay boards/native_sim.overlay` enables emulated RTC, GPIO, and BLE for desktop testing.
